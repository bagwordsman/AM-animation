...
var debug = animationSettings.debug;
// safari message - settings
var safariHead = animationSettings.safariCaveatHeading,
    safariMsg = animationSettings.safariCaveat,
    safariMsgSmall = animationSettings.safariCaveatSmall,
    safariLink = animationSettings.safariCaveatLink;
...
var signSvg = document.querySelector('.stage-two svg');

createGradient(signSvg, 'PoleGradient', [
// add colour stops here
{ offset: '0', 'stop-color': signpostBg, 'stop-opacity': 1 }, { offset: '100%', 'stop-color': signpostBg, 'stop-opacity': 0 }]);
...
function createGradient(svg, id, stops) {
    ...
    stops.forEach(function (value) {
        ...
        var stops = document.createElementNS(svgNS, 'stop');
        // console.log(stops); note: html is added by the below code
        for (var attr in attributes) {
            if (attributes.hasOwnProperty(attr)) stops.setAttribute(attr, attributes[attr]);
        }
...
// ____________________________________________________________________________________
// safari mobile caveat message
// - a shorter message is shown for mobile devices, similar to end cta button
// - requires height to be set with js for toggle hide to work
// - similar to cta button text for small / large screens
// https://stackoverflow.com/questions/3007480/determine-if-user-navigated-from-mobile-safari

// get the user agent string. Note these may need updating in the future.
var userAgent = window.navigator.userAgent;

// if safari iPad, iPhone, iPod:
if (userAgent.match(/iP(ad|hone|od).+Version\/[\d\.]+.*Safari/i)) {
    var safariText = function safariText() {

        // small text substitute
        if (window.innerWidth < 660) {
            var smText = document.querySelector('.safariMobile .container').getAttribute('data-small');
            var currentText = document.querySelector('.safariMobile .container p').textContent;

            // only run once per resize:
            if (smText !== currentText) {
                document.querySelector('.safariMobile .container p').textContent = smText;
            }
        }

        // large text
        else {
                var lgText = document.querySelector('.safariMobile .container').getAttribute('data-big');
                var _currentText = document.querySelector('.safariMobile .container p').textContent;
                // only run once per resize:
                if (lgText !== _currentText) {
                    document.querySelector('.safariMobile .container p').textContent = lgText;
                }
            }

        // set height - for slideToggle to work. Base this on inner .container height
        var height = document.querySelector('.safariMobile .container').scrollHeight;
        document.querySelector('.safariMobile').style.height = height + 'px';
    };
    // run on load


    // compose the mobile message
    // - create container div
    var safariMobMsg = document.createElement('div');
    safariMobMsg.classList.add('safariMobile');
    // - add data attrs for the content. safariText function determines the content based on screen size
    safariMobMsg.innerHTML = '<div class="container" data-small="' + safariMsgSmall + '" data-big="' + safariMsg + '">\n        <h3>' + safariHead + '</h3>\n        <p>' + safariMsg + '</p><p><a href="' + safariLink + '">' + safariLink + '</a>.</p>\n        <i class="fa fa-2x fa-times"></i>\n    </div>';
    body.appendChild(safariMobMsg);

    var removeBtn = document.querySelector('.safariMobile .fa-times');
    ...
    // - set height of .safariMobile to natural height - for slideToggle to work
    window.onresize = safariText;
    safariText();
} // end safari conditional
...
if (window.addEventListener) window.addEventListener("load", scrollMagic, false);else if (window.attachEvent) window.attachEvent("onload", scrollMagic);else window.onload = scrollMagic;

function scrollMagic() {

    // css animated elements
    var miamTicks = Array.from(document.querySelectorAll(".stage-one .tick"));
    var mediationSigns = Array.from(document.querySelectorAll(".stage-two svg > g"));
    var documentElements = Array.from(document.querySelectorAll(".stage-three svg > g.signature, .stage-three svg > g.stamp"));
    ...
    }).setTween(overViewTween).setPin(".overview") // set animation to this section
    ...
    // heading 
    stageTitleTween.fromTo(".stages-title h3", 0.5, {
        opacity: 0,
        transform: "translatex(-500px)"
    }, {
        opacity: 1,
    ...
    var stageTitleScene = new ScrollMagic.Scene({
        ...
        triggerHook: 0
    }).setTween(stageTitleTween).setPin(".stages-title").addTo(stageTitleController);
    ...
    var miamScene = new ScrollMagic.Scene({
        ...
        duration: "2000px", // was 600%
        triggerHook: 0
    }).setTween(miamTween).setPin(".stage-one").addTo(miamController);
    ...
    var mediationScene = new ScrollMagic.Scene({
        ...
        duration: "2000px", // was 600%
        triggerHook: 0
    }).setTween(mediationTween).setPin(".stage-two").addTo(mediationController);
    ...
    var endScene = new ScrollMagic.Scene({
        ...
        duration: "2000px", // was 600%
        triggerHook: 0
    }).setTween(endTween).setPin(".stage-three").addTo(endController);
    ...
    var ctaScene = new ScrollMagic.Scene({
        ...
        triggerHook: 0
    }).setTween(ctaTween).setPin(".end-cta").addTo(ctaController);
